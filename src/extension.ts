...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
	 ...
	 * @author BakxY
	 */
	var disposable = vscode.commands.registerCommand('vhdl-qqs.analyseElaborateCurrentProject', async () => {
		// Get currently active project
		const activeProject: string | null = await pathUtils.getCurrentQuartusProject(context);
		if (activeProject === null) { return; }
		...
		// Run compile command
		compileCommands.analyseQuartusProject(context, activeProject, path.normalize(quartusPath));
	});
	context.subscriptions.push(disposable);
